...
This function detects zero/negative values and replaces them
...
*data* SE a SummarizedExperiment object of gene/abundance counts.

*group_colname* a string specifying the column name in the colData(SE)
representing the sample group.
*remove.sparse.features* a logical argument (TRUE/FALSE) for removal of
...
a SummarizedExperiment object with adjusted zeros. Features with a sum of zero counts
exceeding the smallest sample group size are removed and the remaining zeros are replaced.

**An example run:**

```{r, eval = TRUE, echo = TRUE, results='hide'}
# Create an object with fixed zeros. A dataset without NA values ("campp2_brca_se_1_replacedNAs" generated in a previous step) is used as an input.
campp2_brca_se_1_zeroFix<-FixZeros(campp2_brca_se_1_replacedNAs, group_colname="subtype", remove.sparse.features=TRUE)
```
